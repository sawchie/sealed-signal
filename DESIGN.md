...
On desktop, one two-row command bar sits above the gallery. The shelf heading and count occupy the top left, with the saved-opening preference above the right-hand buttons. Filters/sorting/view controls and All products/Saved/buying-signal controls share the second row, aligned through CSS subgrid without fixed heights or a divider. The routine browser-storage note is removed from the catalog; actual storage-failure messages remain visible. Below 1100px the groups stack in reading order. The hero search and product-type buttons retain their original left edge. The active Filters button uses the purple selected-control treatment with white text; the drawer explicitly uses a navy surface and dark native controls, preventing old light-theme colors from leaking through.
