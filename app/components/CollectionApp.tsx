...
function ValueHistory({ points }: { points: CollectionSnapshot[] }) {
  ...
  return <section className={styles.history} aria-labelledby="collection-history-title"><h2 id="collection-history-title"><span className={styles.historyTitle}>Value history</span><button type="button" className={styles.historyToggle} aria-expanded={historyOpen} aria-controls={historyContentId} onClick={() => setHistoryOpen(open => !open)}>Value history <svg viewBox="0 0 16 16" aria-hidden="true" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="m5 3 5 5-5 5" /></svg></button> <Info label="value history">Recorded on visits and edits, once per UTC day. Adds, sales and quote coverage change the total. This is collection value, not investment return. Missing quotes leave gaps.</Info></h2><div id={historyContentId} className={styles.historyContent} data-open={historyOpen}>
...
function LotRow({ lot, state, product }: { lot: CollectionLot; state: CollectionState; product?: CollectionProduct }) {
  ...
  return <article className={styles.lot}>
    ...
    {error && <p role="alert" className={styles.error}>{error}</p>}<p role="status" className={message === "Paid per unit saved." ? "sr-only" : styles.feedback}>{message}</p>
  </article>;
}
    ...
    {!ready ? <p role="status">Loading your collection from this browser…</p> : <>
      ...
      <div className={styles.collectionLayout}>
        <div className={styles.holdings}>
          ...
          <div className={styles.listContext}><span>{tab === "sold" ? `${sales.length} ${sales.length === 1 ? "sale" : "sales"}` : `${lots.length} ${lots.length === 1 ? "purchase" : "purchases"}`}</span><span><Info label="row values">Paid and Market are per unit; Value and Est. gain cover the row’s held quantity. Estimated gain is before selling costs. Manage opens cost, quantity and sale controls. Unknown values show a dash.</Info></span></div>
