...
export function SiteNavigation() {
  ...
  return <><nav className="site-nav" aria-label="Primary navigation">{items}{pathname !== "/admin" && <CurrencySelector />}</nav><details className="mobile-navigation" onKeyDown={e => { if (e.key === "Escape") { e.currentTarget.open = false; e.currentTarget.querySelector("summary")?.focus(); } }}><summary>Menu</summary><nav aria-label="Mobile navigation">{items}{pathname !== "/admin" && <CurrencySelector />}</nav></details></>;
}
